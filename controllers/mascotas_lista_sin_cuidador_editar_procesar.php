...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (validarEditarMacotasSinAsignar($id, $nombre, $especie, $edad, $genero, $estado, $peso) == false) {
        header("Location: /adopcioncom/views/mascotas_lista_sin_cuidador_editar.php?id=$id&msj=" . urlencode("Datos invalidos ingresados") . "&tipo=error");
        exit();
    }
    editarMascotaSinAsignar($pdo, $nombre, $especie, $edad, $genero, $estado, $peso, $id);
    header("Location: /adopcioncom/views/mascotas_lista_sin_cuidador.php?msj=" . urlencode("Mascota actualizada exitosamente"));
    exit();
}
else {
