...
function obtenerMascotaPorIdBD($pdo, $id)
{
    try {
...
function obtenerMascotaSinAsignarPorIdBD($pdo, $id)
{
    try {
...
function obtenerMisMascotasPaginadasBD($pdo, $cedula, $inicio, $por_pagina)
{
    try {
        $inicio = (int)$inicio;
...
function contarMisMascotasBD($pdo, $cedula)
{
    try {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM mascotas WHERE cedula_empleado_encargado = ?");
        $stmt->execute([$cedula]);
...
function obtenerMascotasPaginadasBD($pdo, $inicio, $por_pagina)
{
    try {
        $inicio = (int)$inicio;
        $por_pagina = (int)$por_pagina;
...
function contarMascotasBD($pdo)
{
    try {
        return $pdo->query("SELECT COUNT(*) FROM mascotas")->fetchColumn();
    }
    catch (PDOException $e) {
        die("Error en la consulta: " . $e->getMessage());
    }
...
function obtenerMascotasSinAsignarPaginadasBD($pdo, $inicio, $por_pagina)
{
    try {
        $inicio = (int)$inicio;
        $por_pagina = (int)$por_pagina;
...
function contarMascotasSinAsignarTotalBD($pdo)
{
    try {
        return $pdo->query("SELECT COUNT(*) FROM mascotas_sin_asignar")->fetchColumn();
    }
    catch (PDOException $e) {
        die("Error en la consulta: " . $e->getMessage());
    }
