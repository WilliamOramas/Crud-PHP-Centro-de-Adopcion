...
$titulo = "Editar Mascota sin Asignar - Pequeños Amigos";
include('header.php');
...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $mascota = obtenerMascotaSinAsignarPorIdBD($pdo, $id);
}
...
<main class="max-w-7xl mx-auto p-6 lg:p-10">
    ...
    <div class="mb-10">
        <h2 class="serif text-4xl font-bold text-brand-dark">Editar Mascota (Sin Asignar)</h2>
    ...
    <div class="bg-white rounded-[2rem] shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden max-w-2xl">
        <form action="/adopcioncom/controllers/mascotas_lista_sin_cuidador_editar_procesar.php" method="POST" class="p-8 lg:p-12 space-y-6">
            <input type="hidden" name="id" value="<?= $mascota['id_mascotas_sin_asignar'] ?>">
            ...
            <div class="pt-6 flex gap-4">
                ...
                <a href="mascotas_lista_sin_cuidador.php" class="px-8 bg-gray-100 text-gray-400 font-bold py-4 rounded-2xl hover:bg-gray-200 hover:text-gray-600 transition-all text-center">
                    Cancelar
                </a>
            </div>
        </form>
